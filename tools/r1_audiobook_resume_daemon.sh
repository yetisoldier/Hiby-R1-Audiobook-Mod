...
catalog_field_for_path() {
  ...
  catalog_path=$2
  [ -f "$CATALOG" ] || return 1
  grep -F "$catalog_path" "$CATALOG" | head -1 | awk -F '\t' -v n="$catalog_field" '{ print $n; exit }'
}
...
catalog_field_for_root_index() {
  ...
  catalog_index=$3
  [ -f "$CATALOG" ] || return 1
  awk -F '\t' -v root="$catalog_root" -v index="$catalog_index" -v n="$catalog_field" '
    $1 == root && $2 == index { print $n; exit }
  ' "$CATALOG"
}
...
book_title_direct_track_select() {
  ...
  swipes=0
  if [ "$saved_index" -gt 5 ]; then
    swipes=$(( (saved_index - 5 + 3) / 4 ))
  fi
  [ "$swipes" -le "$BOOK_TITLE_DIRECT_TRACK_MAX_SWIPES" ] || {
    log "direct-track-select too many swipes saved=$saved_index max=$BOOK_TITLE_DIRECT_TRACK_MAX_SWIPES"
    return 1
  }
  row=$((saved_index - (swipes * 4)))
