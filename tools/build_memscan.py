#!/usr/bin/env python3
"""Build the MIPS memory scan helper for the HiBy R1.

Cross-compiles ``tools/r1_audiobook_memscan.c`` using the Zig C compiler
(``zig cc -target mipsel-linux-musleabi``).  The output is a static MIPS ELF
binary suitable for the R1 device.

Converted from ``tools/build_r1_memscan_helper.ps1``.
"""
...
DEFAULT_OUT_FILE = "work/native-memscan/r1_audiobook_memscan"
...
def build(args: argparse.Namespace) -> Path:
    ...
    # Source file.
    source = REPO_ROOT / "tools" / "r1_audiobook_memscan.c"
